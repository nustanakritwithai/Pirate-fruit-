...
3. Confirm `client/dist/` contains zero MP3 and zero GLB files.
4. Confirm built JavaScript contains no `data:audio/...;base64` payload.
5. Confirm the false-flag build has no audio button or `AudioContext`; both flag states must make zero MP3/GLB requests.

### Staging-only verification

Set `VITE_ENABLE_AUDIO_SYSTEM=true` only on a non-production Static Site and rebuild.

1. Open DevTools Network, filter `mp3` and `glb`, then play through island and sailing: transfer must remain zero before and after unlock.
...
1. Clear Network log and reload on mobile data or throttled Fast 3G; verify zero MP3/GLB requests throughout play.
...
5. Sail long enough to change procedural score states and observe memory. Only current/crossfading oscillator layers should remain; voice cap is 10 on touch/mobile devices.

### Monitoring

Audio warnings are prefixed `[audio]` and must remain non-fatal. Track JS transfer, memory during sailing crossfade and reports of locked audio; MP3/GLB transfer must remain zero. An audio failure is not permission to alter combat, monster, boat or server authority.
