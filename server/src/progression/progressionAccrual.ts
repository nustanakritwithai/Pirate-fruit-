...
export async function accrueServerExp(
  ...
  await client.query(
    `insert into player_progression (character_id) values ($1)
  ...
  await client.query(
    'update player_progression set exp = $2, updated_at = now() where character_id = $1',
    [characterId, String(walked.exp)],
  );
